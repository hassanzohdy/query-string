...
description: End-to-end usage patterns for @mongez/query-string — URL-driven filters, pagination, multi-select facets, safe round-trips, server-side parsing, store sync, and composing with sibling packages.
when_to_use: User is building URL-driven filter/sort/pagination UI with @mongez/query-string, user needs a complete working example combining parse and serialize, user asks how to safely handle special characters in query params, user wants to sync URL state with a store or use the package server-side.
---
