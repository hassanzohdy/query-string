 ...
 *
 * `@mongez/query-string` currently has no runtime `@mongez/*` siblings,
 * but the helper is kept so future deps slot in without touching CI
 * plumbing.
 */
...
export default defineConfig({
  test: {
    // `queryString.all()` defaults to `window.location.search`, and
    // `queryString.update()` calls `window.history.replaceState`, so the
    // tests need a DOM. happy-dom is the lightest option.
